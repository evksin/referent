...
export async function POST(request: NextRequest) {
  try {
    ...
    // Проверяем, предоставлены ли уже данные или нужно парсить URL
    let parsedData: { title: string; content: string; date: string };

    if (providedParsedData) {
      // Используем предоставленные данные (из файла)
      parsedData = providedParsedData;
    } else {
      // Парсим URL
      if (!url || typeof url !== "string") {
        return NextResponse.json(
          {
            error: "URL_OR_FILE_REQUIRED",
            message: "Необходимо указать URL статьи или загрузить файл.",
          },
          { status: 400 }
        );
      }
      ...
      if (
        !actionType ||
        !["summary", "theses", "telegram"].includes(actionType)
      ) {
        return NextResponse.json(
        ...
        return NextResponse.json(
          {
            ...
            message:
              error.message || "Не удалось загрузить статью по этой ссылке.",
            type: "FETCH_ERROR",
          },
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Не удалось извлечь контент статьи. Возможно, статья недоступна или имеет нестандартную структуру.",
        },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (!openRouterResponse.ok) {
        ...
        if (openRouterResponse.status === 401) {
          ...
          errorMessage =
            "Сервис OpenRouter временно недоступен. Попробуйте позже.";
        } else {
          try {
        ...
        return NextResponse.json(
          {
            error: "TIMEOUT",
            message:
              "Превышено время ожидания ответа от AI (более 2 минут). Статья может быть слишком длинной.",
            type: "TIMEOUT",
          },
          { status: 504 }
        );
...
function getPromptsForActionType(
  ...
  switch (actionType) {
    case "summary":
      return {
        systemPrompt:
          "Ты опытный журналист и редактор, специализирующийся на создании кратких и информативных резюме статей. Твоя задача - выделить суть материала, сохранив все важные детали и контекст. Пиши на русском языке, используй ясный и понятный стиль.",
...
${textToProcess}`,
    ...
    case "theses":
      return {
        systemPrompt:
          "Ты профессиональный аналитик и исследователь, который умеет структурировать информацию и выделять ключевые идеи из текстов. Твоя задача - создать четкий и логичный список основных тезисов. Пиши на русском языке.",
...
${textToProcess}`,
    ...
    case "telegram":
      return {
        systemPrompt:
          "Ты профессиональный SMM-специалист и копирайтер, создающий вирусные и привлекательные посты для Telegram каналов. Ты умеешь балансировать между информативностью и развлекательностью, используя эмодзи, хештеги и правильное форматирование. Пиши на русском языке.",
